...
const API_BASE = process.env.EXPO_PUBLIC_API_BASE; // Match Chat.tsx
...
export default function ProgressPage() {
    ...
    const [data, setData] = useState<{ recent: any[]; weekly: any[]; distribution: any[]; daily: any[] } | null>(null);
    ...
    const fetchStats = async () => {
        try {
            ...
            console.log(`[Stats] Fetching from: ${API_BASE}/api/emotion/stats`);
            const res = await fetch(`${API_BASE}/api/emotion/stats`, {
                headers: {
            ...
            try {
                ...
                console.error("[Stats] JSON Parse Error. Body starts with:", text.substring(0, 50));
                setLoading(false);
                return;
            }

            console.log("[Stats] Successfully fetched data points:", json.recent?.length || 0);
            setData(json);
    ...
    const processChartData = (recent: any[]) => {
        if (!recent || recent.length === 0) {
            console.log("[Stats] No recent emotions found, keeping default chart state.");
            return;
        }
    ...
    const getInsightText = () => {
        if (!data?.recent || data.recent.length === 0) return "Your journey is just beginning.";
        ...
        if (hasHappy && !hasSadness) return "You’ve been moving gently toward light.";
        if (hasSadness) return "Things have been heavy lately, but you're still here.";
                ...
                <ScrollView contentContainerStyle={styles.content}>
                    ...
                    <View style={styles.card}>
                        <View style={styles.cardHeaderRow}>
                            ...
                            <View style={styles.toggleContainer}>
                                <TouchableOpacity
                                    ...
                                    style={[styles.toggleBtn, graphType === 'line' && styles.toggleBtnActive]}
                                >
                                    <Ionicons name="stats-chart" size={16} color={graphType === 'line' ? '#fff' : '#8d44ff'} />
                                </TouchableOpacity>
                                <TouchableOpacity
                                    onPress={() => setGraphType('bar')}
                                    style={[styles.toggleBtn, graphType === 'bar' && styles.toggleBtnActive]}
                                >
                                    <Ionicons name="bar-chart" size={16} color={graphType === 'bar' ? '#fff' : '#8d44ff'} />
                                </TouchableOpacity>
                            </View>
                        </View>
                        ...
                        {loading ? (
                            ...
                            <BarChart
                                ...
                                chartConfig={{
                                    ...
                                    backgroundGradientTo: "#ffffff",
                                    decimalPlaces: 0,
                                    color: ((opacity = 1) => `rgba(141, 68, 255, ${opacity})`) as any,
                                    labelColor: (opacity = 1) => `rgba(0, 0, 0, ${opacity})`,
                                    barPercentage: 0.6,
                                }}
                                style={{ marginVertical: 8, borderRadius: 16 }}
                        ...
                        <View style={{ marginTop: 10, alignItems: 'center' }}>
                            <Text style={{ color: '#6a1b9a', fontSize: 12, fontStyle: 'italic' }}>
                                {graphType === 'line' ? "Wave of your recent emotions" : "Emotion Intensity Profile"}
                            </Text>
                        </View>
                    </View>
                    ...
                    <View style={[styles.card, styles.insightCard]}>
                        <Ionicons name="sparkles" size={20} color="#8d44ff" style={{ marginRight: 8 }} />
                    ...
                    {!loading && data?.weekly?.map((week: any, index: number) => {
                        ...
                        if (dom === 'happy') reflection = "More ease in your words this week.";
                        if (dom === 'sad') reflection = "Things felt heavier.";
                        if (dom === 'anxious') reflection = "A lot on your mind recently.";

                        return (
                            <View key={index} style={styles.weekCard}>
                                <View style={styles.weekHeader}>
                                    <Text style={styles.weekTitle}>Week {weekNum}</Text>
                                    <Text style={styles.weekDate}>{new Date(week.date).toLocaleDateString()}</Text>
                                </View>
                    ...
                    {!loading && (!data?.weekly || data?.weekly.length === 0) && (
                        <Text style={styles.emptyText}>No weekly data yet. Chat more to see reflections!</Text>
                    )}

                    {/* 4. Emotion Distribution (Pie Chart) */}
                    <Text style={[styles.sectionTitle, { marginTop: 20 }]}>Emotion Mix</Text>
                    <View style={styles.card}>
                        {loading ? <ActivityIndicator color="#8d44ff" /> : (
                            <PieChart
                                data={getPieData()}
                        ...
                        {(!data?.distribution || data.distribution.length === 0) && !loading && (
                            <Text style={{ color: '#888' }}>No data to show distribution.</Text>
                        )}
                    </View>
                        ...
                        {loading ? <ActivityIndicator color="#8d44ff" /> : (
                            <BarChart
                                ...
                                chartConfig={{
                                    ...
                                    decimalPlaces: 0,
                                    color: ((opacity = 1) => `rgba(141, 68, 255, ${opacity})`) as any, // Purple bars
                                    labelColor: (opacity = 1) => `rgba(0, 0, 0, ${opacity})`,
                                    barPercentage: 0.7,
                                }}
                                style={{
